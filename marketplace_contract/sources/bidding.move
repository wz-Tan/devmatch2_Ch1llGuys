...
// auction
const AUCTION_MINIMUM_DURATION: u64 = 86400; // minimum auction duration of one day
...
public entry fun createBidding(
    ...
    if (option::is_some(&auction.highestBidID)){
        ...
        let highest_bid: &Bid = auction.bidBag.borrow(auction.highestBidID);
...
public entry fun globalTimedTasks(
    ...
    while (index < length) {
        ...
        if (option::is_some(&highestBidID)){
            ...
            nft.addAuctionRecord(uid_to_inner(&bid_record.id));
            transfer::share_object(bid_record);

...
function CollectionSection(): JSX.Element {
  return (
    <section className="px-8 py-12 my-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {collections.map((collection, index) => (
          <div
            key={index}
            className="bg-[rgb(4,4,4)] rounded-xl overflow-hidden hover:bg-gray-950 transition-colors"
          >
            <div className="grid grid-cols-3 gap-1 mb-4">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="aspect-square bg-gradient-to-br from-orange-400 to-red-600"
                ></div>
              ))}
            </div>
            <div className="p-4">
